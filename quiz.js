...
const Quiz = (() => {
  ...
  function media(q) {
    return `${q.image ? `<figure class="quiz-media"><img src="${esc(q.image)}" alt="${esc(q.imageAlt || 'Hình minh họa câu hỏi')}" loading="lazy"><figcaption>${esc(q.imageAlt)}</figcaption></figure>` : ''}${q.productUrl ? `<a class="quiz-product-link" href="${esc(q.productUrl)}" target="_blank" rel="noopener noreferrer">Xem sản phẩm tham khảo ↗</a>` : ''}`;
  }
  ...
  function renderResult(r) {
    displayedResult = r;
    root().innerHTML = `<div class="quiz-panel"><p>${esc(r.name)} · ${esc(r.email)}</p><div class="quiz-score">${r.score}/100 — ${esc(r.status)}</div><p>${r.pendingCount ? `Điểm tạm tính · ${r.pendingCount} câu tự luận chờ chấm` : `Đúng ${r.correctCount}/50 câu`} · ${new Date(r.submittedAt).toLocaleString('vi-VN')}</p><p class="quiz-meta">Kết quả đã lưu. Success: từ 80 điểm; Failed: dưới 80 điểm.${r.gradedAt ? ` Chấm bởi ${esc(r.gradedBy)} · ${new Date(r.gradedAt).toLocaleString('vi-VN')}` : ''}</p><button class="btn btn-secondary" data-quiz="home">Làm bài mới</button></div><h2>Đáp án & giải thích</h2>${r.questions.map((q, i) => {
      const points = r.points ? r.points[i] : q.correct === r.answers[i] ? 2 : 0;
      return `<div class="quiz-panel quiz-review ${points === null ? 'quiz-pending' : points === 2 ? 'quiz-correct' : 'quiz-wrong'}"><div class="quiz-question-head">${numberTag(i, q.type)}<span class="quiz-points">${points === null ? 'Chờ chấm' : points + ' điểm'}</span></div><h3>${esc(q.prompt)}</h3>${media(q)}<p>Bạn trả lời: ${esc(q.type === 'paragraph' ? r.answers[i] : q.options[r.answers[i]])}</p>${q.type === 'paragraph' ? '<strong>Đáp án tham khảo / hướng dẫn chấm</strong>' : `<p>Đáp án đúng: <strong>${esc(q.options[q.correct])}</strong></p>`}<p>${esc(q.explanation)}</p><a href="#${esc(q.source)}" data-source="${esc(q.source)}">Nguồn: ${esc(sources[q.source] || q.source)}</a>${q.type === 'paragraph' && meta.canManage ? `<label>Chấm câu tự luận<select data-grade="${esc(q.id)}"><option value="">Chọn đánh giá</option><option value="2" ${points === 2 ? 'selected' : ''}>Đúng · 2 điểm</option><option value="0" ${points === 0 ? 'selected' : ''}>Sai · 0 điểm</option></select></label>` : ''}</div>`;
    }).join('')}${meta.canManage && r.questions.some(q => q.type === 'paragraph') ? '<div class="quiz-sticky"><span>Chấm đủ câu tự luận để hoàn tất kết quả</span><button class="btn btn-primary" data-quiz="grade">Lưu điểm</button></div>' : ''}`;
  }
  async function saveGrades() {
    const grades = {};
    for (const el of root().querySelectorAll('[data-grade]')) {
      if (el.value === '') throw Error('Vui lòng chấm đủ câu tự luận.');
  ...
  document.addEventListener('change', async e => {
    ...
    if (el.dataset.imageUpload) {
      ...
      finally { busy = false; el.disabled = false; root().querySelectorAll('button').forEach(button => button.disabled = false); }
    }
  });
  document.addEventListener('click', async e => {
    ...
    try {
      ...
      else await ({ home: load, start, submit, results, bank: loadBank, save, create, saveQuestion, grade: saveGrades, addOption: () => { if (draft.options.length >= 8) throw Error('Tối đa 8 lựa chọn.'); draft.options.push(''); renderCreate(); }, removeOption: () => { if (draft.options.length <= 2) throw Error('Cần ít nhất 2 lựa chọn.'); draft.options.pop(); draft.correct = Math.min(draft.correct, draft.options.length - 1); renderCreate(); } }[el.dataset.quiz])();
    } catch (e) { error(e); }
