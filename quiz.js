...
const Quiz = (() => {
  ...
  const sources = { websites: 'Websites', 'cach-lam-viec': 'Cách làm việc', 'bao-gia': 'Tư vấn & Báo giá', 'tao-don': 'Follow đơn', 'bao-hang-ve': 'Báo hàng về', 'hang-stock': 'Hàng stock', faq: 'FAQ', 'thuat-ngu': 'Thuật ngữ', 'noi-quy': 'Nội quy chung' };
  ...
  function renderAttempt() {
    root().innerHTML = `<p class="quiz-meta">${esc(attempt.name)} · 50 câu · 2 điểm/câu. </p><p class="quiz-meta">${Object.entries({short:"trả lời ngắn",mc:"trắc nghiệm",paragraph:"tự luận",tf:"đúng/sai"}).map(([t,n])=>attempt.questions.filter(q=>q.type===t).length+" "+n).join(" · ")}</p><button class="btn btn-secondary" data-quiz="restart">Bắt đầu đề mới</button>${attempt.questions.map((q, i) => `<fieldset class="quiz-panel quiz-question" id="quiz-q-${i}"><legend class="quiz-sr-only">Câu ${i + 1}: ${esc(questionText(q))}</legend><div class="quiz-question-head">${numberTag(i, q)}<span class="quiz-points">2 điểm</span></div>${media(q)}${['paragraph', 'short'].includes(q.type) ? `<label>Câu trả lời của bạn<${q.type === 'short' ? 'input type="text" value="' + esc(answers[i]) + '"' : 'textarea'} data-answer="${i}" data-paragraph="true" maxlength="5000" placeholder="Trình bày câu trả lời…">${q.type === 'short' ? '' : esc(answers[i]) + '</textarea>'}</label><p class="quiz-meta">Admin hoặc Editor sẽ chấm theo đáp án tham khảo.</p>` : q.options.map((o, j) => `<label class="quiz-choice"><input type="radio" name="quiz-answer-${i}" data-answer="${i}" value="${j}" ${(answers[i]?.option ?? answers[i]) === j ? 'checked' : ''}><span class="quiz-option-letter">${String.fromCharCode(65 + j)}</span><span>${esc(o)}</span></label>${q.otherOption === j ? `<input type="text" data-other-answer="${i}" maxlength="5000" aria-label="Mục khác" placeholder="Mục khác…" value="${esc(answers[i]?.text)}">` : ''}`).join('')}</fieldset>`).join('')}<div class="quiz-sticky"><span id="quiz-progress">Đã trả lời ${answers.filter(answered).length}/50</span><button class="btn btn-primary" data-quiz="submit">Nộp bài</button></div>`;
  }
  async function refreshTest() {
    if (busy || !attempt || !root()?.querySelector('[data-quiz="submit"]') || document.hidden) return false;
    const data = await api();
    if (busy || !attempt || !root()?.querySelector('[data-quiz="submit"]')) return false;
    if (data.refreshError) throw Error(data.refreshError);
    if (data.active && data.active.bankRevision !== attempt.bankRevision) {
      attempt = data.active; answers = Array(50).fill(null); renderAttempt();
      document.getElementById('quiz-error').textContent = 'Ngân hàng đã cập nhật. Đề đã tự làm mới; vui lòng trả lời đề mới.';
      return true;
    }
    return false;
  }
  setInterval(() => refreshTest().catch(error), 5000);
  window.addEventListener('focus', () => refreshTest().catch(error));
  async function submit() {
    const data = await api();
    if (data.refreshError) throw Error(data.refreshError);
    if (data.active && data.active.bankRevision !== attempt.bankRevision) { attempt = data.active; answers = Array(50).fill(null); renderAttempt(); throw Error("Ngân hàng đã cập nhật. Vui lòng trả lời đề mới."); }
    ...
    const result = await api('/attempts/' + attempt.id + '/submit', { method: 'POST', body: JSON.stringify({ answers, bankRevision: attempt.bankRevision }) });
    try { sessionStorage.removeItem('wiki-quiz-' + attempt.id + '-' + attempt.bankRevision); } catch {}
  ...
  function renderResult(r) {
    ...
    root().innerHTML = `<div class="quiz-panel"><p>${esc(r.name)} · ${esc(r.email)}</p><div class="quiz-score">${r.score}/100 — ${esc(r.status)}</div><p>${r.pendingCount ? `Điểm tạm tính · ${r.pendingCount} câu chờ chấm` : `Đúng ${r.correctCount}/50 câu`} · ${new Date(r.submittedAt).toLocaleString('vi-VN')}</p><p class="quiz-meta">Kết quả đã lưu. Success: từ 80 điểm; Failed: dưới 80 điểm.${r.gradedAt ? ` Chấm bởi ${esc(r.gradedBy)} · ${new Date(r.gradedAt).toLocaleString('vi-VN')}` : ''}</p><button class="btn btn-secondary" data-quiz="home">Làm bài mới</button></div><h2>Đáp án & giải thích</h2>${r.questions.map((q, i) => {
      const points = r.points ? r.points[i] : (['short', 'paragraph'].includes(q.type) || q.correct === null) ? null : q.correct === r.answers[i] ? 2 : 0;
      return `<div class="quiz-panel quiz-review ${points === null ? 'quiz-pending' : points === 2 ? 'quiz-correct' : 'quiz-wrong'}"><div class="quiz-question-head">${numberTag(i, q)}<span class="quiz-points">${points === null ? 'Chờ chấm' : points + ' điểm'}</span></div>${media(q)}<p>Bạn trả lời: ${esc(['paragraph', 'short'].includes(q.type) ? r.answers[i] : typeof r.answers[i] === 'object' ? q.options[r.answers[i].option] + ': ' + r.answers[i].text : q.options[r.answers[i]])}</p>${(['short', 'paragraph'].includes(q.type) || q.correct === null) ? '<strong>Hướng dẫn chấm thủ công</strong>' : `<p>Đáp án đúng: <strong>${esc(q.options[q.correct])}</strong></p>`}<p>${esc(q.explanation)}</p><a href="#${esc(q.source)}" data-source="${esc(q.source)}">Nguồn: ${esc(sources[q.source] || q.source)}</a>${(['short', 'paragraph'].includes(q.type) || q.correct === null) && meta.canManage ? `<label>Chấm câu trả lời<select data-grade="${esc(q.id)}"><option value="">Chọn đánh giá</option><option value="2" ${points === 2 ? 'selected' : ''}>Đúng · 2 điểm</option><option value="0" ${points === 0 ? 'selected' : ''}>Sai · 0 điểm</option></select></label>` : ''}</div>`;
    }).join('')}${meta.canManage && r.questions.some(q => (['short', 'paragraph'].includes(q.type) || q.correct === null)) ? '<div class="quiz-sticky"><span>Chấm đủ câu cần duyệt để hoàn tất kết quả</span><button class="btn btn-primary" data-quiz="grade">Lưu điểm</button></div>' : ''}`;
  }
  ...
  async function saveQuestion() {
    ...
    if (root().querySelector('[data-bank-row]') && bank && revision === data.revision) {
      renderBank(); document.getElementById('quiz-bank-state').textContent = 'Đã tạo câu hỏi. Các câu khác đang sửa vẫn cần lưu riêng.';
  ...
  async function saveEditedQuestion(index) {
    const q = bank[index];
    const data = await api('/questions/' + encodeURIComponent(q.id), { method: 'PUT', body: JSON.stringify({ question: q, revision }) });
    revision = data.revision;
    bank[index] = data.question;
    changedTypes.delete(q.id);
    const row = root().querySelector('[data-bank-row="' + index + '"]');
    row.querySelector('.quiz-summary-prompt').textContent = questionText(data.question);
    const main = document.getElementById('main'), scrollTop = main.scrollTop;
    filterBank();
    main.scrollTop = scrollTop;

    document.getElementById('quiz-bank-state').textContent = 'Đã lưu câu hỏi';
  }
  document.addEventListener('input', e => {
    ...
    if (el.dataset.otherAnswer !== undefined) {
      ...
      try { sessionStorage.setItem('wiki-quiz-' + attempt.id + '-' + attempt.bankRevision, JSON.stringify(answers)); } catch {}
    }
    ...
    if ((el.dataset.bank !== undefined || el.dataset.draft) && !el.dataset.imageUpload && !el.dataset.removeImage && !el.hasAttribute('data-question-type')) {
      ...
      } else q[el.dataset.field] = el.dataset.field === 'correct' ? (el.value === '' ? null : Number(el.value)) : el.value;
  ...
  document.addEventListener('click', async e => {
    ...
    const el = e.target.closest('[data-quiz], [data-result], [data-source], [data-remove-image], [data-delete-question], [data-save-question]');
    if (!el || busy) return;
    ...
    try {
      if (el.dataset.saveQuestion !== undefined) await saveEditedQuestion(Number(el.dataset.saveQuestion));
      else if (el.dataset.deleteQuestion !== undefined) {
        const q = bank[Number(el.dataset.deleteQuestion)];
        if (!confirm('Xóa câu hỏi này khỏi ngân hàng? Bài chưa nộp sẽ cập nhật; kết quả đã nộp được giữ nguyên.')) return;
        const data = await api('/questions/' + encodeURIComponent(q.id), { method: 'DELETE', body: JSON.stringify({ revision }) });
        const main = document.getElementById('main'), scrollTop = main.scrollTop;
        const deletedIndex = Number(el.dataset.deleteQuestion);
        // Keep existing DOM (including open editors and loaded images) to avoid scroll jumps.
        const rows = [...root().querySelectorAll('[data-bank-row]')];
        const nextRow = rows.slice(deletedIndex + 1).find(row => !row.hidden);
        const previousRow = rows.slice(0, deletedIndex).reverse().find(row => !row.hidden);
        const anchor = nextRow || previousRow;
        const targetY = Math.max(main.getBoundingClientRect().top + 16, Math.min(rows[deletedIndex].getBoundingClientRect().top, main.getBoundingClientRect().bottom - 80));
        bank.splice(deletedIndex, 1); revision = data.revision; changedTypes.delete(q.id);
        rows[deletedIndex].remove();
        root().querySelectorAll('[data-bank-row]').forEach((row, i) => {
          row.dataset.bankRow = i;
          row.querySelector('.quiz-number').textContent = String(i + 1).padStart(2, '0');
          row.querySelectorAll('[data-bank]').forEach(field => field.dataset.bank = i);
          row.querySelector('[data-delete-question]').dataset.deleteQuestion = i;
          row.querySelector('[data-save-question]').dataset.saveQuestion = i;
        });
        document.getElementById('quiz-bank-totals').textContent = `${bank.length} câu · ` + ['mc', 'tf', 'paragraph', 'short'].map(t => `${bank.filter(q => q.type === t).length} ${typeName(t).toLowerCase()}`).join(' · ');
        filterBank();
        if (anchor) main.scrollTop += anchor.getBoundingClientRect().top - targetY;
