...
const Quiz = (() => {
  ...
  const numberTag = (i, q) => `<span class="quiz-number" aria-label="Câu ${i + 1}">${String(i + 1).padStart(2, '0')}</span><div class="quiz-heading-copy"><h3>${esc(questionText(q))}</h3><span class="quiz-type">${typeName(q.type)}</span></div>`;
  function media(q) {
  ...
  function renderBank() {
    root().innerHTML = `<div class="quiz-bank-heading"><div><h2>Ngân hàng câu hỏi</h2><p>${bank.length} câu · ${bank.filter(q => q.type === 'mc').length} trắc nghiệm · ${bank.filter(q => q.type === 'tf').length} đúng/sai · ${bank.filter(q => q.type === 'paragraph').length} tự luận · ${bank.filter(q => q.type === 'short').length} trả lời ngắn</p></div><button class="btn btn-primary" data-quiz="create">Tạo câu hỏi</button></div><p class="quiz-meta">50 câu hỏi lấy từ <a href="https://forms.gle/PKSwfk6NTz7dqw167" target="_blank" rel="noopener noreferrer">form Test SALE/CSKH</a>; đáp án chưa được công khai nên cần chấm thủ công hoặc đặt đáp án chuẩn. Bật “Đưa vào đề” để cho phép rút ngẫu nhiên. Cần ít nhất 50 câu đang chọn. Bài đã bắt đầu giữ nguyên câu hỏi và đáp án.</p><div class="quiz-bank-filters"><label>Tìm câu hỏi<input id="quiz-bank-search" type="search" placeholder="Nhập nội dung hoặc mã câu"></label><label>Loại câu hỏi<select id="quiz-bank-type"><option value="">Tất cả loại câu hỏi</option>${['mc', 'tf', 'paragraph', 'short'].map(t => `<option value="${t}">${typeName(t)}</option>`).join('')}</select></label></div><p id="quiz-bank-count" class="quiz-meta" role="status">Hiển thị ${bank.length}/${bank.length} câu</p>${bank.map((q, i) => `<details class="quiz-panel" data-bank-row="${i}"><summary class="quiz-bank-summary"><span class="quiz-number">${String(i + 1).padStart(2, '0')}</span><span class="quiz-summary-copy"><span class="quiz-summary-prompt">${esc(questionText(q))}</span><span class="quiz-type">${typeName(q.type)}${q.enabled ? '' : ' · Đã tắt'}</span></span><span class="quiz-chevron" aria-hidden="true">⌄</span></summary>${questionFields(q, `data-bank="${i}"`)}<a href="#${esc(q.source)}" data-source="${esc(q.source)}">Đối chiếu nguồn wiki</a></details>`).join('')}<div class="quiz-sticky"><span id="quiz-bank-state">Chưa có thay đổi</span><button class="btn btn-primary" data-quiz="save">Lưu ngân hàng</button></div>`;
  }
  function filterBank() {
  ...
  document.addEventListener('change', async e => {
    ...
    if (el.id === 'quiz-create-type') {
      ...
      draft.correct = ['paragraph', 'short'].includes(el.value) ? null : 0; renderCreate();
    }
